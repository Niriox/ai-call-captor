...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Incoming call (twilio-incoming) from:', from, 'to:', to, 'CallSid:', callSid);
    ...
    console.log('Bland AI call initiated (incoming):', blandData);

    // Respond to Twilio with TwiML while Bland AI takes over
...
<Response>
  ...
  <Dial>${blandData.call_id || from}</Dial>
    ...
    console.error('Error in twilio-incoming:', error);
